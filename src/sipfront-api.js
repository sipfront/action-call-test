...
const httpm_auth = require('@actions/http-client/lib/auth.js')
 ...
 */
async function run_call_test(public_key, secret_key, name, destination, sf_environment) {
  ...
  const api_creds = new httpm_auth.BasicCredentialHandler(public_key, secret_key);
  ...
  let sf_res = await httpc.postJson(
    api_base + api_path,
    data
  )
  console.log(sf_res)
  ...
  return new Promise(resolve => { resolve(res.result.run) })
}
